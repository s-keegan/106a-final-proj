...
if __name__ == '__main__':
  ...
  # image capture service: ImageSrv is the service type
    ...
    while b_ar2_tf == None and b_ar2_rot == None:
    	listener.waitForTransform('base', 'ar_marker_4', rospy.Time(0), rospy.Duration(4.0))
        listener.waitForTransform('base', 'ar_marker_4', rospy.Time(), rospy.Duration(4.0))
        # b_ar_trans, b_ar_rot = listener.lookupTransform('base', 'ar_marker_4', rospy.Time(0))
        # if listener.canTransform('base', 'ar_marker_4', rospy.Time.now()):
        ...
        #     print(b_ar_trans)
        #     print(b_ar_rot)
        b_ar2_tf, b_ar2_rot = listener.lookupTransform('base', 'ar_marker_4', rospy.Time(0))
        # br.sendTransform(b_ar_trans, b_ar_rot, rospy.Time(0), "base", "ar_fixed_frame")

    while ar_ar2_tf == None and ar_ar2_rot == None:
    	listener.waitForTransform('ar_marker_4', 'ar_marker_4', rospy.Time(0), rospy.Duration(4.0))
        listener.waitForTransform('ar_marker_4', 'ar_marker_4', rospy.Time(), rospy.Duration(4.0))
        # b_ar_trans, b_ar_rot = listener.lookupTransform('base', 'ar_marker_4', rospy.Time(0))
        # if listener.canTransform('base', 'ar_marker_4', rospy.Time.now()):
        #     print("found fixed")
        #     print(b_ar_trans)
        #     print(b_ar_rot)
        ar_ar2_tf, ar_ar2_rot = listener.lookupTransform('ar_marker_4', 'ar_marker_4', rospy.Time(0))
        # br.sendTransform(b_ar_trans, b_ar_rot, rospy.Time(0), "base", "ar_fixed_frame")

    b_ar_tf2 = b_ar_tf
    b_ar_tf2[3] += (ar_ar2_tf[3] / 2.0)

    while not rospy.is_shutdown():
        try:
            br.sendTransform(b_ar_tf, b_ar_rot, rospy.Time.now(), "ar_fixed_frame", "base")
            br.sendTransform(b_ar_tf2, b_ar_rot), rospy.Time.now(), "ar_fixed_frame", "base")
        ...
        except tf.LookupException as t:
