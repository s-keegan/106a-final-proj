...
if __name__ == '__main__':
  ...
  # ar_broadcaster = tf.TransformBroadcaster()
  points = None
  ...
  while not rospy.is_shutdown():
    try:
      ...
      # ar_broadcaster.sendTransform(translation,rotation,rospy.Time.now(), "base", "ar_marker_fixed")

      matrix_b_hc = get_transform("base", "head_camera")
      matrix_b_ar = get_transform("base", "ar_marker_4")
      matrix_ar_b = get_transform("ar_marker_4", "base")
      ...
      for contour in filtered:
          ...
          planed_point2_in_ar = matrix_ar_b.dot(planed_point2)
          if planed_point2_in_ar[0] < 0.60 and planed_point2_in_ar[0] > -0.04:
            if planed_point2_in_ar[1] < 0.45 and planed_point2_in_ar[1] > -0.04:
              ...
              #   dists = [(val[0]-p.x)**2 + (val[1]-p.y)**2 for val in points]
              #   if min(dists) > 0.05:
              #     print(min(dists))
              #     points.append((p.x, p.y))
              #     pub_centroids.publish(p)
